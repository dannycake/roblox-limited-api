...
const updateMarketplaceData = async () => {
    const collectibles = (await db.getItems() || [])
        .sort(a => a.marketplace ? 1 : -1);

    const promises = [];

    for (const collectible of collectibles) {
        promises.push((async () => {
            const marketplaceData = await roblox.getMarketplaceInfo(collectible.id);
            if (!marketplaceData) return;

            if (JSON.stringify(collectible.marketplace) !== JSON.stringify(marketplaceData))
    ...
    console.log('Finished updating marketplace info')
}
